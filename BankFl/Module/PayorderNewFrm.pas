...
type
  TPaydocForm = class(TForm)
    ...
    procedure TypeLabelDblClick(Sender: TObject);    //Добавлено Меркуловым
  private
...
begin
  ...
  {ClientBtnClick(nil);}
  PayerComboBox.DroppedWidth := 380;
  OpComboBox.DroppedWidth := 480;
...
//Показывает истинный номер док-та у корреспондента
procedure TPaydocForm.TypeLabelDblClick(Sender: TObject);
begin
  KorrIdLabel.Visible := not KorrIdLabel.Visible;
end;
