...
uses
  ...
  Forms,
  Common,
  ImpMarksFrm in 'ImpMarksFrm.pas' {ImpMarksForm};

procedure CreateChildForm(Sender: TObject);
begin
  if ImpMarksForm=nil then
    ImpMarksForm := TImpMarksForm.Create(Sender as TComponent)
  else
    ImpMarksForm.Show;
end;
...
begin
  ...
  with Result do begin
    Caption := '&Пометки импорта';
    Hint := 'Показывает записи-пометки о загруженных проводках';
    GroupIndex := 4;
