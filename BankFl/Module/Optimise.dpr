...
uses
  ...
  Controls,
  Common,
  Optimfrm in 'Optimfrm.pas' {OptimForm};

procedure CreateChildForm(Sender: TObject);
begin
  if OptimForm = nil then
  begin
    OptimForm := TOptimForm.Create(Sender as TComponent);
    OptimForm.ShowModal;
  end;
end;
...
begin
  ...
  with Result do
  begin
    Caption:='Оп&тимизация баз';
    Hint:='Удаляет пакеты закрытых абонентов';
    GroupIndex := 4;
