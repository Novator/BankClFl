...
uses
  ...
  Forms,
  Common,
  TransportsFrm in 'TransportsFrm.pas' {TransportsForm},
  TransFrm in 'TransFrm.pas' {TransForm};

procedure CreateChildForm(Sender: TObject);
begin
  if TransportsForm=nil then
    TransportsForm := TTransportsForm.Create(Sender as TComponent)
  else
    TransportsForm.Show;
end;
...
begin
  ...
  begin
    Caption := 'К&лиринг';
    Hint := 'Задает список банков работающих не через РКЦ';
    GroupIndex := 4;
