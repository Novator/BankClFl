...
begin
  Application.MessageBox(PChar(S), 'Message', 0);
end;
...
begin
  Reg1 := nil;
  if OpenBtrBase(Reg1, 'Base\registr.btr', baNormal) then
  begin
    FillChar(Ident, SizeOf(Ident), #0);
    StrPLCopy(Ident, 'NDS', SizeOf(Ident)-1);
    Len := SizeOf(OldParRec);
    Res := Reg1.GetEqual(OldParRec, Len, Ident, 1);
    if Res=0 then
    begin
      if OldParRec.pmType=ftFloat then
        OldParRec.pmFltValue := 0.18;
      Res := Reg1.Update(OldParRec, Len, Ident, 1);
      if Res=0 then
      begin
        ShowProto('Ok! UpdOldReg: Новое значение: '+FloatToStr(OldParRec.pmFltValue));
        BitBtn1.Enabled := False;
      end
      else
        ShowProto('UpdOldReg: BtrErr='+IntToStr(Res));
    end
    else
      ShowProto('GetOldReg: BtrErr='+IntToStr(Res));
  end;
  CloseReg(Reg1);
end;
          ...
          4: {RegMove}
            begin
              ...
              begin
                ...
                if RegMoveMode=0 then
                  Err := not MoveRegParamsByName(Reg1, Reg2, S, True)
                    {MoveOldReg(S?)}
                else
                  Err := not MoveRegParamsByName(Reg1, Reg2, S, True);
                    {MoveNewReg(S);}
