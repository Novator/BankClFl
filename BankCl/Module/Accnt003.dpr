...
begin
  ...
  begin
    Caption := '&Счета';
    Hint := 'Показывает состояние счетов и движения по ним, строит выписку и ведомость остатков';
  ...
  case Reason of
    DLL_PROCESS_ATTACH:
      begin
        ...
        ObjList.Add(DLLList);
        BillFormList := TList.Create;
        ObjList.Add(BillFormList);
        LoadDLLs;
      end;
    DLL_PROCESS_DETACH:
      begin
        I := BillFormList.Count;
        while I>0 do
        begin
          try
            Dispose(BillFormList.Items[I-1]);
          except
          end;
          Dec(I);
        end;
