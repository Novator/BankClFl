...
uses
  ...
  Utilits,
  CommCons,
  ValAccountsFrm in 'ValAccountsFrm.pas' {ValAccountsForm};
...
begin
  if ValAccountsForm = nil then
    ValAccountsForm := TValAccountsForm.Create(Sender as TComponent)
  else
    ValAccountsForm.Show;
end;
...
begin
  ...
  with Result do
  begin
    Caption := '&Валютные счета';
    Hint := 'Показывает состояние валютных счетов и движения по ним';
