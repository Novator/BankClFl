...
uses
  ...
  ClntCons,
  DocFunc,
  PokupkaValFrm in 'PokupkaValFrm.pas' {PokupkaValForm};

procedure GetDocuments(AList: TStringList);
begin
  AList.Add('101=Платежное поручение');
  AList.Add('102=*Платежное требование');
  AList.Add('116=*Платежный ордер');
...
begin
  ...
  GetRegParamByName('CharAfterNazn', CharAfterNazn);
  GetRegParamByName('EnterAfterNazn', EnterAfterNazn);
  GetRegParamByName('WithNDSRem', WithNDSRem);
  GetRegParamByName('WithoutNDSRem', WithoutNDSRem);
  GetRegParamByName('CharAfterRem', CharAfterRem);
  PokupkaValForm := TPokupkaValForm.Create(Sender);
  with PokupkaValForm do
  begin
    ...
    begin
      ...
      case PayRecPtr^.dbDoc.drType of
        102: Caption := 'Платежное требование';
      ...
      DecodeDocVar(PayRecPtr^.dbDoc, PayRecPtr^.dbDocLen, Number,
        DebitRs, DebitKs, DebitBik, DebitInn, DebitName, DebitBank,
        CreditRs, CreditKs, CreditBik, CreditInn, CreditName, CreditBank,
        Purpose, DebitKpp, CreditKpp, Status, Kbk, Okato, OsnPl, Period, NDoc,
        DocDate, TipPl, 0, 0, CorrRes, False);
    ...
    repeat
      Err := 0;
      Result := (ShowModal = mrOk) and not ReadOnly;

      if Result and (PayRecPtr<>nil) then
      begin
                  ...
                  begin
                    ...
                    begin
                      ...
                      EncodeDocVar(True, Number,
                        ...
                        CreditRs, CreditKs, CreditBik, CreditInn, CreditName, CreditBank,
                        Purpose, DebitKpp, CreditKpp, Status, Kbk, Okato, OsnPl, Period, NDoc,
                        DocDate, TipPl, 0, 0, CorrRes, False, False, nil, nil,
                        PayRecPtr^.dbDoc, I);
                      PayRecPtr^.dbDocLen := I;
